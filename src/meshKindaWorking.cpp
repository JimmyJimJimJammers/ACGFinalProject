...
Vertex* Mesh::AddEdgeVertexDisp(Vertex *a, Vertex *b, Material *m)
{
    Vertex *v = getChildVertex(a,b);
    if (v != NULL) return v;
    //glm::vec3 pos = 0.5f*a->get() + 0.5f*b->get();
...
Vertex* Mesh::AddMidVertexDisp(Vertex *a, Vertex *b, Vertex *c, Vertex *d, Material *m)
{
    ...
    if (std::fabs(maxHeight) < epsilon)
    {
        printf("MidVert: Negligible height difference, no vert should be created.\n");
    }
