...
public:
  ...
  ArgParser(int argc, char *argv[]) {
    ...
    int getRand(int startRange, int endRange)
    {
        endRange++;//to account for modulus, so if someone wanted (0, 3) inclusive they could get 3
        return rand()%(endRange-startRange) + startRange;
    }
  ...
  void DefaultValues()
  {
    ...
    gather_indirect = false;
      numRadiuses = 1;
      if (num_photons_to_shoot != 0)
      {
          avgRadius = num_photons_to_collect/(float)num_photons_to_shoot;
      }
      else
      {
          avgRadius = .25;
