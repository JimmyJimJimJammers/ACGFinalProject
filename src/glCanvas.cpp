...
void GLCanvas::initializeVBOs(){
  ...
  HandleGLError("leaving initilizeVBOs()");
    //printf("Finished initializing VBOs\n");
}
...
void GLCanvas::cleanupVBOs(){
  ...
  bbox.cleanupVBOs();
    //FOR SUBDIV AND TRIANGLES
    mesh->cleanupVBOs();
    //printf("Finished cleaning up VBOs\n");
}
...
glm::vec3 GLCanvas::TraceRay(double i, double j) {
  ...
  // ==================================
    ...
    {
        i = (i + args->getRandf())*(args->width / double (raytracing_divs_x));
        j = (j + args->getRandf())*(args->height / double (raytracing_divs_y));
        
        // Here's what we do with multiple samples per pixel:
        // construct & trace n rays through n random points per pixle
        for (int k = 0; k < args->num_antialias_samples; k++)
        {
            double x = (i-args->width/2.0)/double(max_d)+0.5; //((i - 500)/2) / 500.5       //i/1001 - 500/1001
            double y = (j-args->height/2.0)/double(max_d)+0.5;//((j - 500)/2) / 500.5       //i and j between 0 and 500
            Ray r = camera->generateRay(x,y);
            Hit hit;
            color += (raytracer->TraceRay(r,hit,args->num_bounces))/(float)args->num_antialias_samples;///(float)args->num_antialias_samples;
            // add that ray for visualization
            RayTree::AddMainSegment(r,0,hit.getT());
            //printf("X: %f\tY: %f\tWidth: %d\tHeight: %d\tmax_d: %d\tColor: (%f %f %f) \n", x, y, args->width, args->height, max_d, color.x, color.y, color.z);
        }//*/
    }
    else
    {
        i = (i + 0.5)*(args->width / double (raytracing_divs_x));
        j = (j + 0.5)*(args->height / double (raytracing_divs_y));
...
int GLCanvas::DrawPixel() {
  ...
  glm::vec3 pos4 =  GetPos((raytracing_x  )*x_spacing, (raytracing_y+1)*y_spacing);
    ...
    //printf("raytracing_x: %d\tx_spacing: %f\traytracing_y: %d\ty_spacing: %f\n", raytracing_x, x_spacing, raytracing_y, y_spacing);
  glm::vec3 color = TraceRay(raytracing_x, raytracing_y);
