...
// ==================================================================
// TEXTURE LOOKUP FOR Displacement Value
// ==================================================================
const glm::vec3 Material::getDisplacementValue(float s, float t) const
{
    if (!hasDisplacementMap()) return displacementColor;
    
    assert (displacementImage != NULL);
    
    // this is just using nearest neighbor and could be improved to
    // bilinear interpolation, etc.
    int i = int(s * displacementImage->Width()) % displacementImage->Width();
    int j = int(t * displacementImage->Height()) % displacementImage->Height();
    if (i < 0) i += displacementImage->Width();
    if (j < 0) j += displacementImage->Height();
    assert (i >= 0 && i < displacementImage->Width());
    assert (j >= 0 && j < displacementImage->Height());
    Color c = displacementImage->GetPixel(i,j);
    
    /*for (int g = 0; g<displacementImage->Height(); g++)
    {
        for (int h = 0; h<displacementImage->Width(); g++)
        {
            printf("(%d %d %d)\t", displacementImage->GetPixel(i, j).r, displacementImage->GetPixel(i, j).g, displacementImage->GetPixel(i, j).b);
        }
        printf("\n");
    }
